...
Any new state of Game will show new visual representation for gamer so he knows what happens, Cutscene started, Player was killed, Bossfight started, NPC dialog, entered Freecam mode, etc.  
...
**Flexy.GameFlow** makes those states and transition obvious in code, easy to work with, develop, and test
...
Typical setup is - bootstrap opens Boot GameStage. Boot then drives game loading process and Opens Meta GameStage
...
When battle ends call ```CloseAndDestroy()``` on GameStage and everything that was part of battle (states, resources, scenes) unloaded and battle result returned  
Want to play next battle? just open new one! It is that simple as it must be :)  
  
GameStage always spawned as child of Graph Root and by default live as root GameObjects of DontDestroyOnLoad scene
Also every GameStage has its own GameContext (Services Container)
Regular States spawned on Stage layers. By default "Base" layer, sometimes "Popups" layer or any other layer you create        
...
Typical states of game: MainMenu, Leaderboard, Settings, PlayState, PauseState, WinState  
...
It defines all possible ways, state can be opened (all Open method signatures)
...
transition happens only once from currently active State to new logical Tip     
FlowGraph spawns TransitionRoot on RootNode  

Transition root resolve StateTransition for 2 states prev and next from their common parent and operation will do all logic of transition  
Default StateTransition is InstantTransition that just deactivate old state and active new one in one frame  
...
Transition Root can be locked. This allows to lock graph on some state so any **external** (calls from states before locked one) attempts to change state will open it before locked one in graph   
...
Graph will correctly spawn new node for state open, connect it to graph and call LogicalEvents on state like Open, Close, etc 
